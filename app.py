  ...
  # ------------------ UPDATED DEMO CHARTS (INTERACTIVE & STYLISH) ------------------
    ...
    with col2:
        ...
        <div class="glass-card" style="border-top: 4px solid #6366f1; padding: 20px; height: 100%;">
             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                <h4 style="margin:0; color: #111827; font-weight:700;">😴 Sleep Quality</h4>
...
with tabs[2]:
    ...
    if data:
        ...
        with c_date:
            target_date = st.date_input("Forecast Until", value=datetime.date.today() + datetime.timedelta(days=7))
        with c_action:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("▶ Run Forecast", key="run_fc"):
                days_diff = (pd.to_datetime(target_date).date() - datetime.date.today()).days
        ...
        if st.session_state.get("analysis_results"):
            ...
            # 2. Forecasting Table (New Addition)
            if res.get("forecast_data"):
                st.markdown("#### 📋 Predicted Metrics Table")

                # Convert JSON list back to DataFrame
                forecast_df = pd.DataFrame(res["forecast_data"])

                # Clean up column names for the UI
                forecast_df.columns = ["Date", "Predicted Value", "Min Expected", "Max Expected"]
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Predicted Value": st.column_config.NumberColumn("Predicted HR", format="%.1f bpm"),
                    }
                )

                # Optional: CSV Download for the forecast
                csv = forecast_df.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="📥 Download Forecast CSV",
                    data=csv,
                    file_name=f"forecast_user_{selected_user}.csv",
                    mime="text/csv",
                )
    else:
